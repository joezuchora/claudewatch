...
`scripts/perf.ts`'s fixture is therefore an envelope the product could never have written.

It survives because `readCacheResult` (packages/core/src/cache.ts:113) validates only that
`snapshot`, `fetchedAt`, `display`, and `freshness` **exist** — not their contents. Run against
...
`fetchedAt` at render, and only the block's *presence* is load-bearing. So the invalid
`staleReason` has never had a symptom, and never will until something starts reading it.
...
All 28 current call sites pass that envelope to an in-memory mock, so the version never reaches
...
`SENTINEL_PCT` in warm-up 0. `smoke.test.ts` has the same protection in substance — every case
asserts `42%` in the child's stdout — but spelled as eight unrelated literals rather than one
named value, so nothing connects the seed to the assertions that depend on it.
...
- [ ] The cache version in the seed comes from `CACHE_VERSION`, not a literal, so a bump cannot
      leave a seed behind.
...
- [ ] The seeded utilization is a single named value each consumer *derives* its assertions
      from. `smoke.test.ts` writes `42` once as the seed and seven more times as bare literals
      in `toContain('42%')`; those assertions are already a real read-the-seed guard, so this
      is not a new guard but a removal of the seven ways it can be silently detached from what
      was seeded.
- [ ] Each of the four claims above fails when the guard is removed. Demonstrated by mutation,
