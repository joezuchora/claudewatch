...
verify_duration_outlier:stepTimeout   300s ≤ d < 600s
verify_duration_outlier:multiTimeout  600s ≤ d
```
...
| `stepTimeout`/`multiTimeout` | 600s | 2× `STEP_TIMEOUT_MS` — reachable only if more than one step timed out |
...
| `300_000 ≤ d < 600_000` | `'stepTimeout'` |
| `600_000 ≤ d` | `'multiTimeout'` |
...
gap. A closed set of five strings also keeps the fingerprint §17-safe, which the old
...
| E6 | A duration beyond any plausible run (e.g. 10^7 ms) | `multiTimeout`. The top band is open-ended by design; there is no upper bound to get wrong. |
...
| A2 | Each boundary value lands in the **upper** band: 120_000 ⇒ `elevated`, 240_000 ⇒ `severe`, 300_000 ⇒ `stepTimeout`, 600_000 ⇒ `multiTimeout` |
